...
import com.js.retrofitdownload.Api.DownloadInterceptor;
...
public class DownloadManager implements DownloadProgressListener {
    ...
    private DownloadManager() {
        ...
        outFile = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "test.apk");
    ...
    public synchronized void start(String url) {
        ...
        Retrofit retrofit = new Retrofit.Builder()
                ...
                .build();
        if (apis == null) {
            apis = retrofit.create(DownLoadApis.class);
            info.setApis(apis);
        } else {
            apis = info.getApis();
        }
    ...
    private void downLoad() {
        ...
        //注意info.getReadLength()前后都需要添加"bytes="和"-"
        subscribe = apis.download("bytes=" + info.getReadLength() + "-", info.getUrl())
                /*指定线程*/
